            ...
            inquirer.prompt([{
              ...
              if (answers.submit) {
                ...
                request.post(bro.URL + '/', {json: {email: brostate['email'], code: brostate['code'], entry: { cmd: cmd, msg: contents}}, multipart: true}, function(error, res, body) {
                  if (error !== null || res.statusCode !== 201) {
                    var broEntry = path.join(process.env['HOME'], cmd + '.bro');
                    var i = 1;
                    while (fs.existsSync(broEntry)) {
                      broEntry = path.join(process.env['HOME'], cmd + (i++) + '.bro');
                    }
                    fs.writeFile(broEntry, contents, function() {
                      console.log(chalk.yellow.bgRed.bold('Woops. There was an error! Your entry was saved to ' + broEntry));
                    });
                  }
                  else {
